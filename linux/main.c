...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        for (int i = 1; i < argc; i++) {
            ...
            if ((strcmp(arg, "-i") == 0) || (strcmp(arg, "--input") == 0)) {
                ...
                // Use rm -rf for Linux instead of rd /s /q
                snprintf(delete_command, sizeof(delete_command), "rm -rf %s", frames_folder);
    ...
    {   //convert the video into frames
        ...
        // Use mkdir() with proper Linux permissions (0755)
        struct stat st = {0};
        if (stat(frames_folder, &st) == -1) {
            if (mkdir(frames_folder, 0755) == 0) {
