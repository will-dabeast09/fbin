//gcc -o fbin main.c -I./include -L./lib -limagequant -fopenmp
...
int main(int argc, char *argv[]) {
    ...
    {   //quantize each frame into 256 colors each and write file
        ...
        {   //initialize parallel processing / batch sizing
            //get the sysetm information for batch sizing
            ...
            DWORDLONG mem_limit = totalPhysMem / MEM_LIMIT_DENOM;
        ...
        while (current_frame < total_frames) {//(main loop) process frames
            ...
            for (int i = 0; i < frames_in_batch; i++) {
                ...
                {
                    completed_frames++;
                    if ((completed_frames % 25 == 0) || (completed_frames == total_frames)) {
...
int get_total_frames(const char *frames_folder, const char *frame_name) {
    ...
    snprintf(search_path, MAX_PATH, "%s\\%s_*.png", frames_folder, frame_name);
